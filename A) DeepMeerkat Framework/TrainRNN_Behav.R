...
ArrayNames<- Files[which(endsWith(Files, "Array_Behav.rda"))]
IDNames <- Files[which(endsWith(Files,"ID_Behav.rda"))]

#Get Videos for training
Unmatch <- read.csv("../Data/VideoUnMatchEvents.csv")
Unmatch$Array <- paste(Unmatch$Video,"_RNNArray_Behav.rda", sep="")
Unmatch$ID <- paste(Unmatch$Video,"_RNNID_Behav.rda", sep="")
...
FittedVids <-str_remove(ArrayNames, pattern="_RNNArray_Behav.rda")
...
IDVect <- as.numeric(IDVect) # id vect are strings for some reason, need to convert to flaot
...
#Saving all arrays to save time on clust - B for behaviour
save(MainArray, file="../RNNArrays/MainArray_B.rda")
save(IDVect, file="../RNNArrays/IDVect_B.rda")
save(Val.Array, file="../RNNArrays/Val.Array_B.rda")
save(Val.IDVect, file="../RNNArrays/Val.IDVect_B.rda")

#Loading all of it
load(file="../RNNArrays/MainArray_B.rda")
load(file="../RNNArrays/IDVect_B.rda")
load(file="../RNNArrays/Val.Array_B.rda")
load(file="../RNNArrays/Val.IDVect_B.rda")
...
RNN %>% #layer_conv_lstm_2d(filters=40, kernel_size = c(3,3), 
         #                  input_shape = c(4,3,1280/imagescale,720/imagescale), 
          #                 data_format = "channels_first", return_sequences = TRUE,
           #                activation="relu", padding="same") %>%
  #layer_batch_normalization()%>%
  layer_conv_lstm_2d(filters=40, kernel_size = c(4,4), 
                     input_shape = c(4,3,1280/imagescale,720/imagescale), 
...
#test:
# RNN %>% fit(MainArray, IDVect, epochs=25)
print(dim(MainArray)) #print dimension of array
RNN %>% save_model_hdf5("../Models/TrainedBehavRNN")
