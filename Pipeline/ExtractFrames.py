...
for Vid in chunks[iter]:
    ...
    #read in video:
    RawVid = mpy.VideoFileClip("{Eph}/RawVideos2/{File}".format(Eph=Ephdir, File=VidFile[0]))

    #make directory for vid if not already
    if not os.path.exists("{Eph}/ClipFrames/{Vid}/".format(Vid=Vid,Eph=Ephdir)):
                os.mkdir("{Eph}/ClipFrames/{Vid}/".format(Vid=Vid, Eph=Ephdir))
    ...
    for i in range(len(df.index)): #for each row of dataframe
        ...
        #Extract frame images from each video
        for j in range(len(TimeSeq)):
            if not os.path.exists("{Eph}/ClipFrames/{Vid}/Event{Event}/".format(Vid=Vid,Event=Event, Eph=Ephdir)):
                os.mkdir("{Eph}/ClipFrames/{Vid}/Event{Event}/".format(Vid=Vid,Event=Event, Eph=Ephdir))
            RawVid.save_frame("{Eph}/ClipFrames/{Vid}/Event{Event}/{Frame}.png".format(Vid=Vid,Event=Event,Frame=j, Eph=Ephdir),TimeSeq[j])
